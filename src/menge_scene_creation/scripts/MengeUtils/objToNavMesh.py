...
def analyze_obj(obj_file, vertex_tolerance=1e-4):
    ...
    for i in range(len(obj_file.vertSet) - 1):
        v_i = obj_file.vertSet[i]
        for j in range(i + 1, len(obj_file.vertSet)):
...
def buildNavMesh(objFile, y_up, vertex_distance):
    ...
    edges = edgeMap.keys()
    internal = list(filter(lambda x: len(edgeMap[x]) > 1, edges))
    external = list(filter(lambda x: len(edgeMap[x]) == 1, edges))
