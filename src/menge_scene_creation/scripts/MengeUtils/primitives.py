...
class ColoredVertex(Vertex):
    ...
    def formatPlyBinary(self):
        """Returns a string that represents this vertex in binary PLY format"""
        return Vertex.formatPlyBinary(self) + pack('>BBB', *self.color)
...
class Segment:
    ...
    def normal(self):
        ...
        displacement = self.p2 - self.p1
        segLen = displacement.magnitude()
        if segLen:
            norm = displacement / segLen
    ...
    def pointDistance(self, p):
        ...
        segLen = displacement.magnitude()
        norm = displacement / segLen
        displacementP = p - self.p1
        dp = norm.dot(displacementP)
        if dp < 0:
    ...
    def originDirLen(self):
        ...
        segLen = displacement.magnitude()
        assert (segLen > 0)
        direction = displacement / segLen
        return self.p1, direction, segLen
