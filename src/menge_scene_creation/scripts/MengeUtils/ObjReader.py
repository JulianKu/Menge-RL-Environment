...
FLOAT_EXP = r"[-\+]?[0-9]+\.?[0-9]*e[-\+]?[0-9]+|[-\+]?[0-9]+\.?[0-9]*"
UINT_EXP = "[0-9]+"
# (?:_){0,2} - match 0 to 2 instances of previous expression
VERT_PAT = re.compile(r"\s*v\s+(%s)\s+(%s)\s(%s)" % (FLOAT_EXP, FLOAT_EXP, FLOAT_EXP))
UV_PAT = re.compile(r"\s*vt\s+(%s)\s+(%s)" % (FLOAT_EXP, FLOAT_EXP))
NORM_PAT = re.compile(r"\s*vn\s+(%s)\s+(%s)\s(%s)" % (FLOAT_EXP, FLOAT_EXP, FLOAT_EXP))
FACE_PAT = re.compile(r"\s*f\s+(%s(?:\\%s){0,2})" % (UINT_EXP, UINT_EXP))
MTLLIB_PAT = re.compile(r"\s*mtllib\s+([a-zA-Z_0-9./]+\.[a-zA-Z0-9]{3})")
GRP_PAT = re.compile(r"\s*g\s+([a-zA-Z][a-zA-Z_0-9]*)")
USEMTL_PAT = re.compile(r"\s*usemtl\s+([a-zA-Z][a-zA-Z_0-9()]*)")
...
class Material:
    ...
    def PLYBinaryFormat(self):
        ...
        # for now it simply outputs the diffuse r, g, b values
        r, g, b = self.diffuse
...
class ObjFile:
    class FaceIterator:
        ...
        def __init__(self, objfile):
            self.mesh = objfile
            self.groups = list(objfile.groups.values())
            self.grpIndex = 0
            self.matGrps = list(self.groups[0].materials.values())
            self.matGrpIndex = 0
                ...
                while tryAgain:
                    ...
                    if self.matGrpIndex >= len(self.matGrps):
                        ...
                        if self.grpIndex >= len(self.groups):
                            ...
                            self.matGrps = list(self.groups[self.grpIndex].materials.values())
                    if self.matGrps:
    ...
    def readFileLike(self, file):
        ...
        for line in file:
            ...
            if not line:
                ...
                if match:
                    try:
                        normal = Vector3(match.group(1), match.group(2), match.group(3))
                        self.normSet.append(normal)
                        self.object_line_numbers[normal] = lineNum
                    except:
                ...
                if match:
                    try:
                        texture = Vector2(match.group(1), match.group(2))
                        self.uvSet.append(texture)
                        self.object_line_numbers[texture] = lineNum
                    except:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("ObjReader -- reads wavefront .obj files and processes them")
    parser.add_argument("in", dest="i", help="the wavefront obj file (.obj) to operate on\n"
                                   "no input, no action")
    parser.add_argument("bp", help="file (.ply) to write binary ply data\n"
                                   "by default no ascii ply file written")
    parser.add_argument("ap", help="file (.ply) to write ascii ply data\n"
                                   "by default no binary ply file written")
    parser.add_argument("obj", help="file (.obj) to write triangulated obj data\n"
                                    "defaults to file_t.obj")
    # parser.add_argument("geo", help="file (.geo) to write binary geo data\n"
    #                                 "by default, no geo file written")
    parser.add_argument("nt", help="don\"t triangulate\n"
                                   "without this command line, the obj will be triangulated",
                        action="store_true")
    parser.add_argument("mat", help="Use materials, if available", action="store_true")
    parser.add_argument("uv", help="output uvs", action="store_true")
    parser.add_argument("norm", help="output normals", action="store_true")
    parser.set_defaults(nt=False, uv=False, norm=False)
    args = parser.parse_args(rp.myargv()[1:])

    inputName = args.i

    objTName = args.obj
    asciiPlyName = args.ap
    binaryPlyName = args.bp
    # geoName = args.geo
    noTris = args.nt
    useMat = args.mat
    useUvs = args.uv
    ...
    if objTName is None and asciiPlyName is None and binaryPlyName is None:
